...
(** init a new Sha224 context *)
val init : unit -> ctx

(** Sha224.update ctx data ofs len update a context with data *)
val update : ctx -> string -> int -> int -> unit

(** Sha224.finalize ctx returns a finalized digest *)
val finalize : ctx -> t

(** Return the digest of the given string. *)
val string : string -> t

(** Sha224.substring s ofs len returns the digest of the substring of s starting
at character number ofs and containing len characters. *)
val substring : string -> int -> int -> t

(** If len is nonnegative, Sha224.channel ic len reads len characters from
channel ic and returns their digest, or raises End_of_file if end-of-file is
reached before len characters are read. If len is negative, Sha224.channel ic
